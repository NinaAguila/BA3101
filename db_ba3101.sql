...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 23, 2023 at 02:20 AM
...
INSERT INTO `tbempaccount` (`empid`, `department_id`, `emp_email`, `emp_profile`, `emp_password`) VALUES
(1, NULL, '1-@g.batstate-u.edu.ph', NULL, '1');
...
(1, 'Almario', 'Arvin', 'CICS');

--
-- Triggers `tbempinfo`
--
DELIMITER $$
CREATE TRIGGER `after_insert_empinfo` AFTER INSERT ON `tbempinfo` FOR EACH ROW BEGIN
...
(1, 'Morales', 'Verlon', 'BSIT');

--
-- Triggers `tbstudinfo`
--
DELIMITER $$
CREATE TRIGGER `after_insert_studinfo` AFTER INSERT ON `tbstudinfo` FOR EACH ROW BEGIN
...
(1, 'Hackathon', 'Contest', '2023-11-30', NULL, 2, 'ongoing');
...
-- Events
--
CREATE DEFINER=`root`@`localhost` EVENT `update_event_status` ON SCHEDULE EVERY 1 SECOND STARTS '2023-11-23 01:08:31' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
